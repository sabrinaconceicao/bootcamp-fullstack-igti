...
const saveStateFile = (estado, listaCidades) => {
  const cidadesPertencentes = listaCidades.filter(
...
const findStatesWithMoreCities = () => {
  ...
  const countedStates = states.map((state) => ({
    uf: state.Sigla,
    count: countNumberOfCities(state.Sigla),
  }));
  ...
  /**** PROFESSOR FDP END *****/
  const printStates = sortedStates
    .splice(0, 5)
    .map((state) => `${state.uf} - ${state.count}`);
  console.log(printStates);
  console.log(
    `Quantidade Total de cidades dos estados com mais cidades = ${sum}`
  );
};
...
const findStatesWithLessCities = () => {
  ...
  console.log(printStates);
  console.log(
    `Quantidade total de cidades dos estados com menos cidades = ${sum}`
  );
};
...
const findAllCitiesWithLargestName = () => {
  ...
  const largestNamesCities = states.map(
    (state) => `${findCityWithLargestNameByState(state)} - ${state.Sigla}`
  );
...
const findAllCitiesWithSmallestName = () => {
  ...
  const largestNamesCities = states.map(
    (state) => `${findCityWithSmallestNameByState(state)} - ${state.Sigla}`
  );
...
const findSmallestNameCity = () => {
  ...
  const smallestNamesCities = states.map(
    (state) => `${findCityWithSmallestNameByState(state)} - ${state.Sigla}`
  );
