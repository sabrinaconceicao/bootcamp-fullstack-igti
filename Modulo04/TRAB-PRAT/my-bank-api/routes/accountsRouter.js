...
app.get("/accounts/:agencia/:conta", async (req, res) => {
  try {
    ...
    const accounts = await accountsModel.findOne({
  ...
  try {
    ...
    if (!accountOrigin) {
      ...
      if (accountOrigin.agencia === accountDestiny.agencia) {
        const newAccountOrigin = await accountsModel.findOneAndUpdate(
          { conta: params.origin },
          { $inc: { balance: -params.value } }
        );

        const newAccountDestiny = await accountsModel.findOneAndUpdate(
          { conta: params.destiny },
          { $inc: { balance: params.value } }
        );
...
app.get("/accounts/less", async (req, res) => {
  try {
    ...
    res.send(
      "Operação realizada com sucesso! Você pode ver o resultado no console."
    );
  } catch (err) {
    res.status(500).send(err);
  }
...
app.patch("/accounts/transfer-client", async (req, res) => {
  try {
    ...
    res.send(
      "Ação concluída com sucesso! Você pode ver o resultado no console."
    );
  } catch (err) {
    res.status(500).send(err);
  }
